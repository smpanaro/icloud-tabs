...
var pos = 0; // position in the array buffer
var buffer;

function append_string(s) {
...
function write_bytes(byte_arr) {
  for (var i = 0; i < byte_arr.length; i++) {
    write_byte(byte_arr[i]);
  }
}
...
function plistFromDicts(dicts) {
  buffer = new Uint8Array(1000);
  append_string("bplist00");
  ...
  function writeTrailer() {
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeTrailer');
    }
    // 6 null bytes
    write_bytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);


    // size of an offset
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeTrailer(offsetSizeInBytes):', offsetSizeInBytes);
    }
    writeByte(offsetSizeInBytes);

    // size of a ref
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeTrailer(offsetSizeInBytes):', idSizeInBytes);
    }
    writeByte(idSizeInBytes);

    // number of objects
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeTrailer(number of objects):', entries.length);
    }
    writeLong(entries.length);

    // top object
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeTrailer(top object)');
    }
    writeLong(0);

    // offset table offset
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeTrailer(offset table offset):', offsetTableOffset);
    }
  ...
  function writeOffsetTable() {
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeOffsetTable');
    }
  ...
  function writeDict(entry) {
    if (debug) {
      ...
      console.log('0x' + pos.toString(16), 'writeDict', '(id: ' + entry.id + ')', '(keys: ' + keysStr + ')', '(values: ' + valsStr + ')');
    }
  ...
  function writeNumber(entry) {
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeNumber', entry.value, ' (type: ' + entry.type + ')', '(id: ' + entry.id + ')');
    }
  ...
  function writeArray(entry) {
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeArray (length: ' + entry.entries.length + ')', '(id: ' + entry.id + ')');
    }
  ...
  function writeBoolean(entry) {
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeBoolean', entry.value, '(id: ' + entry.id + ')');
    }
  ...
  function writeString(entry) {
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeString', entry.value, '(id: ' + entry.id + ')');
    }
  ...
  function writeData(entry) {
    if (debug) {
      console.log('0x' + pos.toString(16), 'writeData', entry.value, '(id: ' + entry.id + ')');
    }
  ...
  function writeDouble(v) {
    ...
    // buffer.write(buf);
    console.log("doubles not supported")
    assert(false);
  }
...
// tab specific stuffs
function magic_tab_header() {
  var buf = new Uint8Array(12)
  var arr = [0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x17, 0x00, 0x00, 0x00, 0x00];
...
function tab_plist_with_header(dicts) {
  var body = plistFromDicts(dicts);
  var header = magic_tab_header();

  var full = new Uint8Array(12 + pos);
  full.set(header, 0);
  full.set(body, 12);
  return full;
}
